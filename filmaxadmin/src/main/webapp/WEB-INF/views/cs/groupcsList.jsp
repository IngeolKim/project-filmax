...
<script>
	$(function(){
		$("#answerNum").change(function(){
			var hrefLink = "/cs/groupcsList?answerNum="+$("#answerNum").val();
			location.href=hrefLink;
		});
		$("#orderNum").change(function(){
			var hrefLink = "/cs/groupcsList?answerNum="+${currAnswer}+"&orderNum="+$("#orderNum").val();
			location.href=hrefLink;
		});
		//날짜 검색 유효성 검사
...
<center>
	<c:if test="${currPage > 1}">
		<a href="/cs/groupcsList?pageNum=${currPage-1}">[이전]</a>
	</c:if>
	<c:forEach begin="1" end="${pageCount}" step="1" var="pageIndex">
		<a href="/cs/groupcsList?pageNum=${pageIndex}">${pageIndex}</a>
	</c:forEach>
	<c:if test="${currPage < pageCount}">
		<a href="/cs/groupcsList?pageNum=${currPage+1}">[다음]</a>
	</c:if>
</center>
<%-- 검색창 --%>
