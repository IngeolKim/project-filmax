...
<center>
	<c:if test="${currPage > 1}">
		<a href="/cs/emailcsDateSearch?startDate=${startDate}&endDate=${endDate}&pageNum=${currPage-1}">[이전]</a>
	</c:if>
	<c:forEach begin="1" end="${pageCount}" step="1" var="pageIndex">
		<a href="/cs/emailcsDateSearch?startDate=${startDate}&endDate=${endDate}&pageNum=${pageIndex}">${pageIndex}</a>
	</c:forEach>
	<c:if test="${currPage < pageCount}">
		<a href="/cs/emailcsDateSearch?startDate=${startDate}&endDate=${endDate}&pageNum=${currPage+1}">[다음]</a>
	</c:if>
</center>
<%-- 검색창 --%>
