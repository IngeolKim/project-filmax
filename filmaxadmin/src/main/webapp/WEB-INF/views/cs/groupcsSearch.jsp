...
<center>
	<c:if test="${currPage > 1}">
		<a href="/cs/groupcsSearch?cName=${cName}&cValue=${cValue}&pageNum=${currPage-1}">[이전]</a>
	</c:if>
	<c:forEach begin="1" end="${pageCount}" step="1" var="pageIndex">
		<a href="/cs/groupcsSearch?cName=${cName}&cValue=${cValue}&pageNum=${pageIndex}">${pageIndex}</a>
	</c:forEach>
	<c:if test="${currPage < pageCount}">
		<a href="/cs/groupcsSearch?cName=${cName}&cValue=${cValue}&pageNum=${currPage+1}">[다음]</a>
	</c:if>
</center>
<%-- 검색창 --%>
